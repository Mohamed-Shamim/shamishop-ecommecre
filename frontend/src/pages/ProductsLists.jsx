...
const ProductLists = () => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            const filtered = productData.filter(product => {
                ...
                const matchesRating = selecteRating.length === 0 || selecteRating.some(r => Math.floor(product.rating) === r)
                const matchesCategories = selecteCategories.length === 0 || selecteCategories.includes(product.category)
                return matchesCategory && matchesSearch && matchesPrice && matchesRating && matchesCategories;
            });
    ...
    }, [location.search, priceRange, productData, selecteRating, selecteCategories]);
    ...
    const handelRatingChange = (rating) => {
        setSelecteRating(prev => prev.includes(rating)
    ...
    const handelCategoryChange = (categorys) => {
        setSelecteCategories(prev => prev.includes(categorys)
    ...
    const visibleProducts = filteredProducts && sortedProducts.slice(0, displayCount);
        ...
        <div className="container mx-auto p-4 min-h-screen">
                    ...
                    onMouseMove={handleMouseMove}>
                    {
                        categories.map(cat => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-full capitalize no-select ${currentCategory === cat
            ...
            <>
                ...
                <div className="flex justify-between">
                    <p className={`text-lg font-semibold flex gap-5 transition duration-300 items-center `} onClick={() => setShowFilter(!showFilter)}>FILTERS <img src={left_arrow} alt="" className={`h-3 transition-all duration-300 ${showFilter ? " rotate-180" : "rotate-90"} `} /> </p>
                ...
                <div className="flex flex-col sm:flex-row gap-1 sm:gap-10 pt-2 ">
                    ...
                    <div className="md:min-w-56 lg:w-80 w-full xl:w-96 sm:w-60  ">
                        ...
                        {/* Category Filter */}
                        <div className={`border border-gray-300 pl-5 py-3 mt-6 max-h-52 overflow-y-auto no-scrollbar ${showFilter ? "" : "hidden"} sm:block`}>
                    ...
                    {isLoading ? (
                        ...
                        <div className="flex-col overflow-y-scroll no-scrollbar items-center justify-center ">
                            <h1 className='text-lg font-semibold items-center flex '>ALL COLLECTIONS</h1>
